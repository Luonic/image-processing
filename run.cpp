...
void read_file_as_uchar_pointer(const char * filename, const unsigned char*& buffer, long& length) {
    ...
    if (f)
    {
        ...
        length = ftell(f);
        fseek (f, 0, SEEK_SET);
        typelessBuffer = malloc(length + 100);
        if (typelessBuffer)
        {
...
int main(int argc, char **argv) {
    ...
    read_file_as_uchar_pointer("/home/alex/Code/image-processing/images/image2.jpg", buffer, length);
    ...
    // std::cout << model_stream.rdbuf() << std::endl;
    // mp.initialize_segmentation_model(model_stream);
    ...
    float background_coloring_strength = 1.0f;
    float background_coloring_angle = 200;
    float background_coloring_saturation = 0.125f;
